...
const placeholder = useVModel(calendarProps, 'modelValue', calendarEmits, {
  passive: true,
...
const dateValue = ref(placeholder.value);
...
onMounted(() => {
    ...
    if (user.date_of_birth) {
        dateValue.value = parseDate(user.date_of_birth);
    }
});
...
const submit = () => {
    if (dateValue.value) {
        form.date_of_birth = dateValue.value.toString();
    }
    ...
    form.post(route('profile.update'), {
        ...
        onSuccess: () => {
            if (form.profile_photo) {
                currentProfilePhotoPreview.value = URL.createObjectURL(form.profile_photo);
                profilePhotoPreview.value = null;
            }
        },
    });
            ...
            <div class="flex flex-col space-y-6">
                ...
                <form @submit.prevent="submit" class="space-y-6">
                    ...
                    <div class="grid gap-2">
                        ...
                        <Select v-model="form.blood_group" class="w-full">
                            <SelectTrigger>
                                ...
                                >
                                    <CalendarHeader>
                                        <CalendarHeading class="flex w-full items-center justify-between gap-2">
                                            <Select
                                                :default-value="dateValue?.month.toString()"
                                                @update:model-value="(v) => {
                                                    if (!v || !dateValue) return;
                                                    if (Number(v) === dateValue?.month) return;
                                            ...
                                            <Select
                                                :default-value="dateValue?.year.toString()"
                                                @update:model-value="(v) => {
                                                    if (!v || !dateValue) return;
                                                    if (Number(v) === dateValue?.year) return;
                                                    dateValue = dateValue.set({
                                                        year: Number(v),
                                                    })
                                                }"
