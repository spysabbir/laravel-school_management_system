...
import { type BreadcrumbItem, type User, type Role, type Designation } from '@/types';
...
const openEditDialog = (user: User) => {
    ...
    form.designation_id = user.teacher?.designation_id?.toString() ?? '';
...
const submit = () => {
    if (editingUser.value) {
        form.put(route('teachers.update', editingUser.value.id), {
            onSuccess: () => {
                ...
                <Dialog v-model:open="dialogOpen">
                    <DialogTrigger as-child>
                        <Button @click="openCreateDialog" class="flex items-center gap-2">
                            <Plus class="h-4 w-4" /> New User ( Teacher )
                        </Button>
                    </DialogTrigger>
                    <DialogScrollContent class="sm:max-w-[425px]">
                        <DialogHeader>
                            <DialogTitle>{{ editingUser ? 'Edit' : 'Create' }} User ( Teacher )</DialogTitle>
                            <DialogDescription>
                        ...
                        <form @submit.prevent="submit" class="p-4">
                            <div class="grid gap-6">
                                ...
                                <Button type="submit" class="mt-4 w-full" :disabled="form.processing">
                                    ...
                                    {{ editingUser ? 'Edit' : 'Create' }} User ( Teacher )
                                </Button>
                            </div>
                        </form>
                        <DialogFooter>
