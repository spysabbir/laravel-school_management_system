...
const form = useForm<{
    ...
    status: user.teacher?.status || 'Running',
    resignation_date: user.teacher?.resignation_date || '',
    retirement_date: user.teacher?.retirement_date || '',
    suspension_date: user.teacher?.suspension_date || '',
    resignation_reason: user.teacher?.resignation_reason || '',
    suspension_reason: user.teacher?.suspension_reason || '',
});

const submit = () => {
    form.put(route('teachers.update', user.id), {
            onSuccess: () => {
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-4 rounded-xl p-4">
            <div class="mb-4 flex items-center justify-end">
                <Link :href="route('teachers.index')" class="inline-flex items-center px-4 py-2 bg-gray-800 dark:bg-gray-200 border border-transparent rounded-md font-semibold text-xs text-white dark:text-gray-800 uppercase tracking-widest hover:bg-gray-700 dark:hover:bg-white focus:bg-gray-700 dark:focus:bg-white active:bg-gray-900 dark:active:bg-gray-300 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 dark:focus:ring-offset-gray-800 transition ease-in-out duration-150">
                    <List />
                </Link>
            </div>
