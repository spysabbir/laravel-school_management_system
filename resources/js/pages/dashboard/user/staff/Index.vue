...
const openEditDialog = (user: User) => {
    ...
    form.designation_id = user.staff?.designation_id?.toString() ?? '';
...
const submit = () => {
    if (editingUser.value) {
        form.put(route('staffs.update', editingUser.value.id), {
            onSuccess: () => {
                ...
                <Dialog v-model:open="dialogOpen">
                    <DialogTrigger as-child>
                        <Button @click="openCreateDialog" class="flex items-center gap-2">
                            <Plus class="h-4 w-4" /> New User ( Staff )
                        </Button>
                    </DialogTrigger>
                    <DialogScrollContent class="sm:max-w-[425px]">
                        <DialogHeader>
                            <DialogTitle>{{ editingUser ? 'Edit' : 'Create' }} User ( Staff )</DialogTitle>
                            <DialogDescription>
                        ...
                        <form @submit.prevent="submit" class="p-4">
                            <div class="grid gap-6">
                                ...
                                <Button type="submit" class="mt-4 w-full" :disabled="form.processing">
                                    ...
                                    {{ editingUser ? 'Edit' : 'Create' }} User ( Staff )
                                </Button>
                            </div>
                        </form>
                        <DialogFooter>
